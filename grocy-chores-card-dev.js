...
class GrocyChoresCard extends s$1 {
        ...
        return x`
            ${x`
                <ha-card>
                    ...
                    <select name="cars" id="carsID" @change=${() => this._setUserId()}>
    ...
    _setUserId(setUser){
    let users = this.shadowRoot.getElementById("carsID");
    this.userId = parseInt(users.value)
  }
...
window.customCards.push({
    type: 'grocy-chores-card-dev',
    name: 'Grocy Chores Card Dev',
    preview: false,
    description: 'A card used to display chores and/or tasks from the Grocy custom component.',
});

customElements.define('grocy-chores-card-dev', GrocyChoresCard);
